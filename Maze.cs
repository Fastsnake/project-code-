...
public class Maze : MonoBehaviour {
	...
	private int currentNeigbour =0;
	public List<int> lastCells;
	public int backingUp =0;
	...
	void CreateMaze(){
		while (visitedCells < totalCells) {
			if (startedBuilding){
				GiveMeNeighbour();
				if (cells[currentNeigbour].visited == false && cells[currentCell].visited ==true){
					BreakWall();
					cells[currentNeigbour].visited = true;
					visitedCells++;
					lastCells.Add(currentCell);
					currentCell = currentNeigbour;
	...
	void GiveMeNeighbour(){
		...
		if (length != 0) {
			...
			currentNeigbour = neighbours [theChosenone];
